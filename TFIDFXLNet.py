...
############################ Load the datasets ############################
training_preprocessed   = pd.read_csv(f'~/csvs/TFIDFtrain{n}.csv')
testing_preprocessed    = pd.read_csv(f'~/csvs/TFIDFtest{n}.csv')
validation_preprocessed = pd.read_csv(f'~/csvs/TFIDFval{n}.csv')
...
# %%
training_preprocessed['selected_words'][0]
...
word_count = count_words(training_preprocessed['selected_words'][0])
...
print('Original: ', training_preprocessed["selected_words"][0])

# Split the sentence into tokens - XLNet
print('Tokenized XLNet: ', xlnet_tokenizer.tokenize(training_preprocessed["selected_words"][0]))

# Mapping tokens to token IDs - XLNet
print('Token IDs XLNet: ', xlnet_tokenizer.convert_tokens_to_ids(xlnet_tokenizer.tokenize(training_preprocessed["selected_words"][0])))

# %%
#assigning merged information and labels to separate variables

selected_words = training_preprocessed["selected_words"].values
...
def xlnet_tokenization(dataset):
  selected_words = dataset["selected_words"].values
...
for epoch_i in range(0, epochs):
    ...
    if ACC > best_acc:
        ...
        torch.save(xlnet_model.state_dict(),f'best_model{n}.pth')
...
# load best model
xlnet_model.load_state_dict(torch.load(f'best_model{n}.pth'))
