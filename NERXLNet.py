...
############################ Load the datasets ############################
training_preprocessed   = pd.read_csv(f'~/csvs/train_NER{n}keywords.csv')
testing_preprocessed    = pd.read_csv(f'~/csvs/test_NER{n}keywords.csv')
validation_preprocessed = pd.read_csv(f'~/csvs/val_NER{n}keywords.csv')
...
# %%
training_preprocessed['NER_words'][0]
...
word_count = count_words(training_preprocessed['NER_words'][0])
...
print('Original: ', training_preprocessed["NER_words"][0])

# Split the sentence into tokens - XLNet
print('Tokenized XLNet: ', xlnet_tokenizer.tokenize(training_preprocessed["NER_words"][0]))

# Mapping tokens to token IDs - XLNet
print('Token IDs XLNet: ', xlnet_tokenizer.convert_tokens_to_ids(xlnet_tokenizer.tokenize(training_preprocessed["NER_words"][0])))

# %%
#assigning merged information and labels to separate variables

NER_words = training_preprocessed["NER_words"].values
...
def xlnet_tokenization(dataset):
  NER_words = dataset["NER_words"].values
...
for epoch_i in range(0, epochs):
    ...
    if ACC > best_acc:
        ...
        torch.save(xlnet_model.state_dict(),f'best_model{T}.pth')
...
# load best model
xlnet_model.load_state_dict(torch.load(f'best_model{T}.pth'))
